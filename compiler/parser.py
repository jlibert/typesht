...
class Parser:
    ...
    def parse_statement(self) -> Any:
        ...
        if tok.type == DEF:      return self.parse_funcdef()
        if tok.type == CLASS:    return self.parse_classdef()
        ...
        if tok.type == BREAK:
            self.advance(); self.match(NEWLINE); return BreakNode()
        if tok.type == CONTINUE:
            self.advance(); self.match(NEWLINE); return ContinueNode()
        if tok.type == PASS:
            self.advance(); self.match(NEWLINE); return PassNode()
    ...
    def parse_primary(self) -> Any:
        ...
        if tok.type == INT:    self.advance(); return NumberNode(tok.value)
        if tok.type == FLOAT:  self.advance(); return NumberNode(tok.value)
        if tok.type == STRING: self.advance(); return StringNode(tok.value)
        if tok.type == BOOL:   self.advance(); return BoolNode(tok.value)
        if tok.type == NONE:   self.advance(); return NoneNode()
        if tok.type == SELF:   self.advance(); return IdentNode("self")
        if tok.type == IDENT:  self.advance(); return IdentNode(tok.value)
        if tok.type == LAMBDA: return self.parse_lambda()
        if tok.type == FSTRING:
            self.advance()
        ...
        if tok.type == LPAREN:
            ...
            self.expect(RPAREN)
            return expr
        if tok.type == LBRACKET: return self.parse_list()
        if tok.type == LBRACE:   return self.parse_dict()
