...
class CodeGen:
    ...
    def gen_BinOpNode(self, node) -> str:
        ...
        if op == "//":      return f"$.floordiv({left}, {right})"
        if op == "%":       return f"$.pymod({left}, {right})"
        if op == "**":      return f"Math.pow({left}, {right})"
        if op == "==":      return f"{left} === {right}"
        if op == "!=":      return f"{left} !== {right}"
        if op == "in":      return f"$.contains({right}, {left})"
        if op == "not in":  return f"!$.contains({right}, {left})"
    ...
    def gen_UnaryOpNode(self, node) -> str:
        ...
        if node.op == "not": return f"!{operand}"
        if node.op == "-":   return f"-{operand}"
    ...
    def gen_BreakNode(self, node):    self.emit("break;")
    def gen_ContinueNode(self, node): self.emit("continue;")
    def gen_PassNode(self, node):     pass
